...
class OpenClawLLMAdapter {
  constructor(options = {}) {
    ...
    this.timeoutSeconds = options.timeoutSeconds ?? (parseInt(process.env.TRIGGER_TIMEOUT_SEC, 10) || 300);
    this.client = null;
  }
  ...
  async send(agentId, message) {
    await this.connect();
    return this.client.sendToAgent(agentId, message, this.timeoutSeconds);
  }
...
class MockLLMAdapter {
  async send(_agentId, message) {
    const body = summarizePrompt(message);
    return `<memory_artifact>${body}</memory_artifact>\nNO_REPLY`;
  }
